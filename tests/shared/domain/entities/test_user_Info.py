...
class Test_UserInfo:
    def test_user_info(self):
        ...
        assert user_info.social_name == "Test"
        assert user_info.certificate_with_social_name is True

    def test_user_info_no_social_name(self):
        user_info = UserInfo(
            user_id="a" * 36,
            name="test",
            role=ROLE.EXTERNAL,
        ...
        assert user_info.accepted_notifications_sms is True
        assert user_info.accepted_notifications_email is True
        assert user_info.social_name is None
        assert user_info.certificate_with_social_name is False
    ...
    def test_user_info_invalid_social_name(self):
        with pytest.raises(EntityError):
            user_info = UserInfo(
                user_id="a" * 36,
                name="test",
