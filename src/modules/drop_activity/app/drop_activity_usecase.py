    ...
    def __call__(self, user_id: str, code: str) -> Enrollment:
        ...
        if taken_slots >= activity.total_slots:
            ...
            if len(in_queue_enrollments) > 0:
                ...
                enrollment = in_queue_enrollments[0]
                updated_enrollment = self.repo.update_enrollment(user_id=enrollment.user.user_id, code=code, state=ENROLLMENT_STATE.ENROLLED)
